...
@:final class ShapeTool extends BitmapTool
{
	...
	private var MIN_SHAPE_SIZE_FOR_OBJECT : Int = 2;
	private var THETA : Int = 45;
	...
	* Stroke width
	*/ 
	private function set_strokeWidth(strokeW : Float) : Float
	{
	...
	private function get_strokeWidth() : Float
	{
		return _strokeWidth;
    } 
	...
	*/
	private function set_strokeColor(color : Int) : Int
	{
		_strokeColor = color;
        return color;
    }
	...
	*/ 
	private function set_fillColor(color : Int) : Int
	{
		_fillColor = color;
        return color;
    }
	...
	* Alpha of the Stroke
	*/  
	private function set_strokeAlpha(alpha : Float) : Float
	{
		_strokeAlpha = alpha;
        return alpha;
    }
	...
	* Alpha of the Fill
	*/ 
	private function set_fillAlpha(alpha : Float) : Float
	{
		_fillAlpha = alpha;
        return alpha;
    }
	...
	*/ 
	public function getShapeDefinition() : ShapeDefinition
	{
		...
		var shapeValidWidth : Bool = false;
		var shapeValidHeight : Bool = false; 
		...
		var shapeDef : ShapeDefinition = null;
	...
	*/
	override public function apply(drawingTarget : DisplayObject, point1 : Point, point2 : Point = null) : Void
	{
        ...
        else if (_type == ShapeType.OVAL) 
		{
			...
			for (k in Math.round(THETA / 2) ... 361)
			{
        ...
        else if (_type == ShapeType.CIRCLE) 
		{
			...
			d = r / Math.cos(Conversions.radians(0.5 * THETA));
			centerPoint = new Point(point1.x + ((xDiff) / 2), point1.y + ((yDiff) / 2));
			commands.push(GraphicsPathCommand.MOVE_TO); drawingData.push(centerPoint.x + r); 
			drawingData.push(centerPoint.y); for (k in Math.round(THETA / 2)...361)
			{
				controlAngleRadians = Conversions.radians(k);
				anchorAngleRadians = Conversions.radians(k + (THETA / 2));
				xControl = d * Math.cos(controlAngleRadians);
